...
{
    jl_lambda_info_t *li = jl_get_specialization1(types);
    if (li && jl_generating_output() && jl_options.incremental && li->def->isstaged)
        return jl_nothing; // TODO: precompile can't deal with this right now
